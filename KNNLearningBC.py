...
class knnLearnerBC():
    ...
    def plot_validation_curve(self, classifier, X, y, param_name, param_range=np.logspace(-6, -1, 5), cv=None):
        ...
        plt.fill_between(param_range, test_scores_mean - test_scores_std,
                         ...
                         color="navy")
        plt.legend(loc="best")
        filename = '{}/images/{}/{}/{}_MCC.png'.format('.', 'BC', self.name, param_name)
    ...
    def learn(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(self.features, self.labels, test_size=0.2, random_state=0, shuffle=True, stratify=self.labels)
        scaler=preprocessing.StandardScaler().fit(X_train)
        X_train=scaler.transform(X_train)
        X_test=scaler.transform(X_test)

        self.classifier = knn(weights='uniform')

        cv = 3;
        self.plot_learning_curve(self.classifier,"Learning curve", X_train,y_train,cv=cv)
        filename = '{}/images/{}/{}/LC.png'.format('.', 'BC', self.name)
        plt.savefig(filename, format='png', dpi=150)
        plt.close()

        self.plot_validation_curve(self.classifier,X_train,y_train,"n_neighbors", np.arange(1,50,1), cv=cv)
        self.plot_validation_curve(self.classifier,X_train,y_train,"metric", ['manhattan', 'euclidean', 'chebyshev'], cv=cv)

        self.classifier.set_params(n_neighbors=9, metric='manhattan')
        self.plot_learning_curve(self.classifier,"Learning curve-with optimised hyperparameter", X_train,y_train,cv=cv)
        filename = '{}/images/{}/{}/LC(optimized).png'.format('.', 'BC', self.name)
        plt.savefig(filename, format='png', dpi=150)
        plt.close()
        ...
        filename = '{}/images/{}/{}/CM.png'.format('.', 'BC', self.name)
        ...
        filename = '{}/images/{}/{}/CM_Normalized.png'.format('.', 'BC', self.name)

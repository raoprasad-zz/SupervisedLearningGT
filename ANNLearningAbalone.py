...
class annLearnerAbalone():
    ...
    def generateFinalLC(self, prefix=''):
        self.plot_learning_curve(self.classifier,
                                 prefix+ "Optimized Learning curve - " + self.datasetName + '-' + self.algoname, self.X_train,
                                 self.y_train,
                                 cv=self.cv)
    ...
    def doGridSearch(self):
        dimension = self.features.shape[1]
        parameters = {'alpha': np.logspace(-5, 3, 20), 'max_iter': [2 ** x for x in range(10)],
         'hidden_layer_sizes': [(h,) * l for l in [1, 2, 3] for h in [dimension, dimension // 2, dimension * 2]]}
        # parameters = {'solver':['lbfgs', 'adam'], "alpha": np.logspace(-5, 3, 20),'max_iter':np.arange(2, 601, 20)}
